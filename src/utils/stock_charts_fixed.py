...
def fetch_stock_data(ticker: str, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    try:
        ...
        if isinstance(data.columns, pd.MultiIndex):
            print("Detected MultiIndex columns - flattening data structure")
            # If we have MultiIndex columns, select only the data for our ticker
            # and convert to a regular DataFrame with standard column names
            standard_columns = {}
            for col_type in ['Open', 'High', 'Low', 'Close', 'Volume']:
                if (col_type, ticker) in data.columns:
                    standard_columns[col_type] = data[(col_type, ticker)]
                elif col_type in data.columns:
                    standard_columns[col_type] = data[col_type]

            # Create a new DataFrame with standard columns
            data = pd.DataFrame(standard_columns, index=data.index)
...
def create_stock_chart(
    ...
    try:
        ...
        # Add Bollinger Bands
        if 'BB' in indicators and all(col in df.columns for col in ['BB_High', 'BB_Mid', 'BB_Low']):
            fig.add_trace(
                go.Scatter(
                    x=df['Date'], 
                    y=df['BB_High'],
        ...
        # Add MACD
        if 'MACD' in indicators and all(col in df.columns for col in ['MACD', 'MACD_Signal', 'MACD_Hist']):
            # MACD Line
            fig.add_trace(
                go.Scatter(
                    x=df['Date'], 
        ...
        print(f"Error in create_stock_chart: {str(e)}")
        import traceback
        traceback.print_exc()
